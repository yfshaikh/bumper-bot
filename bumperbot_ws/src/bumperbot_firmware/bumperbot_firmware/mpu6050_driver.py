...
class MPU6050_Driver(Node):
    ...
    def __init__(self):
        ...
        # I2C interface status; auto-reconnect is handled in timerCallback().
        self.is_connected_ = False
        self.init_i2c()

        # ROS 2 publisher and reusable IMU message buffer.
        ...
        # Timer period in seconds (0.01 s = 100 Hz).
        self.frequency_ = 0.01
        self.timer_ = self.create_timer(self.frequency_, self.timerCallback)

    def timerCallback(self):
        try:
            # Retry sensor initialization if previous I2C access failed.
            if not self.is_connected_:
                self.init_i2c()
            
            # Read accelerometer raw counts (16-bit signed per axis).
            acc_x = self.read_raw_data(ACCEL_XOUT_H)
            acc_y = self.read_raw_data(ACCEL_YOUT_H)
            acc_z = self.read_raw_data(ACCEL_ZOUT_H)
            
            # Read gyroscope raw counts (16-bit signed per axis).
            gyro_x = self.read_raw_data(GYRO_XOUT_H)
            gyro_y = self.read_raw_data(GYRO_YOUT_H)
            gyro_z = self.read_raw_data(GYRO_ZOUT_H)
            
            # Convert raw counts using project-specific scale factors.
            # These constants combine sensor sensitivity + unit conversion.
            ...
            # Stamp and publish latest sample.
            self.imu_msg_.header.stamp = self.get_clock().now().to_msg()
            self.imu_pub_.publish(self.imu_msg_)
        except OSError:
            # Mark disconnected; next timer cycle will attempt re-init.
            self.is_connected_ = False

    def init_i2c(self):
        try:
            # Open I2C bus 1 (common Raspberry Pi bus).
            self.bus_ = smbus.SMBus(1)
            # MPU6050 startup configuration sequence.
    ...
    def read_raw_data(self, addr):
        # Accelerometer and gyroscope values are 16-bit (high byte + low byte).
        high = self.bus_.read_byte_data(DEVICE_ADDRESS, addr)
        low = self.bus_.read_byte_data(DEVICE_ADDRESS, addr+1)
        
        # Combine into unsigned 16-bit value.
        value = ((high << 8) | low)
            
        # Convert from unsigned to signed two's-complement range.
        if(value > 32768):
            value = value - 65536
        return value
